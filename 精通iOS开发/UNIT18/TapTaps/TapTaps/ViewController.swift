...
class ViewController: UIViewController {
    ...
    func singleTap() {
        singleLabel.text = "Single Tap Dectected"
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, Int64(2 * NSEC_PER_SEC)), dispatch_get_main_queue(),
            {self.singleLabel.text = ""})
    }
    
    func doubleTap() {
        doubleLabel.text = "Double Tap Dectected"
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, Int64(2 * NSEC_PER_SEC)), dispatch_get_main_queue(),
            {self.doubleLabel.text = ""})
    }

    func tripleTap() {
        tripleLabel.text = "Triple Tap Dectected"
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, Int64(2 * NSEC_PER_SEC)), dispatch_get_main_queue(),
            {self.tripleLabel.text = ""})
    }

    func quadrupleTap() {
        quadrupleLabel.text = "Quadruple Tap Dectected"
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, Int64(2 * NSEC_PER_SEC)), dispatch_get_main_queue(),
