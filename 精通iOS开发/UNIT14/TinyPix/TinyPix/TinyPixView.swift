...
class TinyPixView: UIView {
    ...
    private func drawBlockAt(row row: Int, column: Int) {
        ...
        let color = document.stateAt(row, col: column)
    ...
    private func toggleSelectedBlock() {
        if selectedBlockIndex.row != -1
            && selectedBlockIndex.column != -1 {
                document.toggleStateAt(selectedBlockIndex.row,
                    col: selectedBlockIndex.column)
                document.undoManager.prepareWithInvocationTarget(document).toggleStateAt(selectedBlockIndex.row, col: selectedBlockIndex.column)
                setNeedsDisplay()
        }
    }
